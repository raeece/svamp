...
boolean inputnumbers2error=false, samenumsp2error=false, odd_mallocerror=false, eoferror=false, unknownerr=false, cursperr=false,
        eofspeciesname=false, illegalchar=false, dotsinbranch=false, plusinbranch=false, dashinbranch=false, einbranch=false,
        openloop=false, commentbracketsserr=false, treenumerr=false, speciesmissing=false; //true if error occurs
...
const char* get_command_name (const char *vector)
{ /* returns the name of the program from vector without the whole path */
  char *last_slash;

  /* Point to the last slash... */
  last_slash = strrchr (vector, DELIMITER);
...
void inputnumbers2(long *spp, long *nonodes, long n)
{
  ...
  if (fscanf(infile, "%ld", spp) != 1 || *spp <= 0) {
    ...
    inputnumbers2error=true;//true if error occurs
  }
  fprintf(outfile, "\n%4ld Populations\n", *spp);
  *nonodes = *spp * 2 - n;
}  /* inputnumbers2 */
...
{ /* error message if attempt to malloc too little or too much memory */
  ...
  odd_mallocerror=true;//true if error occurs
  return;
  //exxit(-1);
}
...
//    if(dotsinbranch || plusinbranch || dashinbranch || einbranch) return;
//    if (q != NULL) {
//      if (!minusread)
...
//      goteof, &nextnode, &ntips, (*no_species), haslengths, unifok, maxnodes);
//  if (dotsinbranch || plusinbranch || dashinbranch || einbranch || speciesmissing) return;
