...
int likemain(FILE * inFileName, FILE * outFileName, FILE * outtree);
...
vector *x;
intvector *reps;
boolean minev, global, jumble, lengths, usertree, fitchlower, upper, negallowed,
        outgropt, replicates, trout, printdata, progress, treeprint,
        mulsets, firstset, incorrectntree=false;
double power;
...
double evaluate(tree *t)
{
  ...
  if ((!minev) && replicates && (fitchlower || upper))
    sum /= 2;
  t->likelihood = -sum;
  return (-sum);
}  /* evaluate */
