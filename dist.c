...
boolean checkFileFormat=true, checkZeroDiagonal=true, checkSymmetric=true;
int unsymcoord1[2], unsymcoord2[2], nonZeroRow;
double unsym[2];
...
int inputdata(boolean replicates, boolean printdata, boolean lower,
                        boolean upper, vector *x, intvector *reps)
{
  ...
  for (i = 0; i < spp; i++) {
    ...
    scan_eoln(infile);
    initname(i);
    if (illegalchar) return -1;
    if (eofspeciesname)return -1;
    for (j = 0; j < spp; j++) {
      ...
      if (!skipit) {
        ...
        if (replicates) {
          ...
          if (fscanf(infile, "%ld", &reps[i][j]) != 1) {
            ...
            checkFileFormat=false;//addition
            return -1;
      ...
      if ((i == j) && (fabs(x[i][j]) > 0.000000001)) {
       //printf("\nERROR: diagonal element of row %ld of distance matrix ", i+1);
        ...
        checkZeroDiagonal=false;//addition
        nonZeroRow = i+1;//addition
        return -1;
      ...
      if ((j < i) && (fabs(x[i][j]-x[j][i]) > 0.000000001)) {
        ...
        unsymcoord1[0]=i+1;unsymcoord1[1]=j+1;
        unsymcoord2[0]=j+1;unsymcoord2[1]=i+1;
        unsym[0]=x[i][j];
        unsym[1]=x[j][i];
        checkSymmetric = false;
        return -1;
...
void treeoutr(node *p, long *col, tree *curtree)
{
  ...
  if (p->tip) {
    ...
    (*col)++;
    if ((*col) > 55) {
      //putc('\n', outtree); //modified so that adhere to newick file format
      (*col) = 0;
    }
  ...
  if (p == curtree->root)
    //fprintf(outtree, ";\n");
    fprintf(outtree, ";");//modified so that adhere to newick file format
  else {
...
void treeout(node *p, long *col, double m, boolean njoin, node *start)
{
  ...
  if (w < 0)
    w = 0;
  if (p == start)
    //fprintf(outtree, ";\n"); //modified so that it adheres to newick file format
    fprintf(outtree, ";");
  else {
    fprintf(outtree, ":%*.5f", (int) w + 7, x);
    *col += w + 8;
